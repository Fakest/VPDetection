...
def lsd_lines(source_image, min_line_length=0.0375, max_line_length=1, min_precision=0):
    ...
    line_lengths = [geom_tools.get_line_length(l) for l in lines]
    # print(f'Line lengths Shape {line_lengths.shape} Length {len(line_lengths)}')
    return [lines[i,:] for i in range(lines.shape[0])
