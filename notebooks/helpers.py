...
def batch_detect_vps_and_score(dataset, detection_func, show_progress_bar=True):
    ...
    for i, image_path in enumerate(image_paths):
        ...
        print(f'Detect lines in {image_path}')
...
def visualize_vp_detection_results(dataset, vp_results, mask_indices=None):
    ...
    for i in mask_indices:
        ...
        print(f"vp result index {i} \n  {vp_results.image_vp_to_lines[i].keys()}")
        plt.rcParams['figure.dpi'] = 300
        plt.rcParams['savefig.dpi'] = 300
        
        fig = plt.imshow(gt_image)
        plt.savefig("/Users/harrybeggs/Documents/Projects/Local-Projects/Python/vp-toolbox-master/ToulouseTruth/" + dataset.image_paths[i][107:-4] + "_gt.png")
        # ax[0].imshow(gt_image)
        # ax[1].imshow(working_image)
        fig = plt.imshow(working_image)
        plt.savefig("/Users/harrybeggs/Documents/Projects/Local-Projects/Python/vp-toolbox-master/ToulouseOutput/" + dataset.image_paths[i][107:-4] + ".png")
        #plt.show()
        gt = "Ground truth: %s" % dataset.image_gt_vps[i]
        result = 'Horizon error: %s. VP angle error (degrees): %s. Line counts: %s. VP detected: %s' % (
            ...
            [len(vs) for vs in vp_results.image_vp_to_lines[i].values()],
             [vp_results.image_vp_to_lines[i].keys()])
        f = open("/Users/harrybeggs/Documents/Projects/Local-Projects/Python/vp-toolbox-master/ToulouseTruth/" + dataset.image_paths[i][107:-4] + "_truth.txt", "w")
        f.write(gt)
        f = open("/Users/harrybeggs/Documents/Projects/Local-Projects/Python/vp-toolbox-master/ToulouseOutput/" + dataset.image_paths[i][107:-4] + ".txt", "w")
        f.write(result)
        f.close
