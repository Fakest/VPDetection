...
def find_vanishing_points_in_image(
    ...
    if x_ransac_options is None:
        print(f'Find VP in image : {lines}')
...
# experiment with parameters, how sensitive are they? how does it effect performance
...
class RansacOptions(object):
    ...
    def __init__(
            ...
            min_inliers=3,
            outlier_rate=0.73,
            desired_success_rate=0.6,
            max_iterations=10, #harry
            random_seed=0):
        if model is None:
...
def _find_vanishing_points_j_linkage(lines, options):
    ...
    print(f'Find Vanishing Points J Linkage function, calculate results : {lines}')
